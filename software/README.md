...
- **エラー表示**: ファームウェアのエラー応答 (`0xEE`) と保護イベント (過電流・過熱など) をステータスバーに 5 秒間表示
- **デバイスパネル** (#29, #47, #49)
  - 全ポートのボードを Scan して一覧から選択 (Auto = 最初に見つかったボード)。ボードの ID に関係なく検出し、その ID に対してコマンドを送る
  - ボード名 (最大 15 バイト) の設定、ファームウェアバージョンの表示
  - 保護機能の有効/無効と閾値の読み書き (`0x20`/`0x21`)
- **位置キャリブレーション** (`0x07`)
